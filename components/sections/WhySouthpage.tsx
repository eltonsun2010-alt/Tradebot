 ...
 * and an editorial glass panel is revealed between them, one principle at
...
const stationZ = (i: number) => -(LEAD + i * GAP_Z);
...
function LightCorridor() {
  ...
  const beams = useMemo<Beam[]>(() => {
    ...
    for (let k = 0; k < count; k += 1) {
      ...
      for (let i = 0; i < N; i += 1) {
        if (Math.abs(z - stationZ(i)) < 780) z += z < stationZ(i) ? -820 : 820;
      }
      arr.push({
        x: side * (150 + rnd() * 300),
        y: (rnd() - 0.5) * 150,
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let activity = 0;
      for (let i = 0; i < N; i += 1) {
        const d = stationZ(i) + camZ; // <0 ahead, 0 = centred, >0 passed
        ...
        if (gl) {
          ...
          gl.style.transform = `translate(-50%, -50%) translate3d(${(-gap).toFixed(1)}px, ${drift.toFixed(1)}px, ${stationZ(i)}px)`;
        }
        if (gr) {
          gr.style.opacity = gateO.toFixed(3);
          gr.style.transform = `translate(-50%, -50%) translate3d(${gap.toFixed(1)}px, ${(-drift).toFixed(1)}px, ${stationZ(i)}px)`;
        }
        if (pn) {
          pn.style.opacity = reveal.toFixed(3);
          pn.style.filter = `blur(${((1 - reveal) * 4).toFixed(2)}px)`;
          pn.style.transform = `translate(-50%, -50%) translate3d(0px, ${(drift * 0.6).toFixed(1)}px, ${stationZ(i) + 140}px)`;
          ...
          <div ref={worldRef} className="relative h-full w-full [transform-style:preserve-3d]">
            ...
            {WHY_CARDS.map((card, i) => (
              <div key={card.title} className="contents">
                ...
                >
                  <Panel index={idx(i)} title={card.title} body={card.body} />
                </div>
              </div>
            ))}
          </div>
